...
Custom GNUastro parameters can be passed with:
...
python3 lisan.py \
    ...
    --filters g,r \
    --make-masks \
    --noisechisel-params="--tilesize=30,30 --dthresh=0.05"
```
...
GNUastro catalogue measurements include `RA`, `DEC`, `MAGNITUDE`, `HALF_SUM_RADIUS`, surface-brightness quantities, areas, S/N, and related size measurements.

The Gaia query requests source ID, sky position, G-band magnitude, parallax, proper motion, and their uncertainties.
...
Diagnostic plots can be disabled with `--cats-no-plots`.
...
## ⚠️ Important Assumptions and Current Limitations

This is a **beta** version. In particular:

1. The preferred input filename is `<GALAXY>_<FILTER>.fits`.
2. The galaxy identifier should not contain additional underscores.
3. The 2D science image should be in HDU 1.
4. HDU 1 should contain a valid celestial WCS.
5. The galaxy name should be resolvable by `SkyCoord.from_name`.
6. Internet access is required for Gaia queries and normally for object-name resolution.
7. PSF building includes manual visual inspection of stellar stamps and is therefore not fully unattended.
8. DS9 is required by the current manual review workflow.
9. The interactive joiner requires a graphical environment.
10. The current `psf_joint.py` plotting configuration uses LaTeX.
11. The release is primarily tested on Linux.
12. `--gals-to-use` currently filters the PSF-joining stage rather than every earlier pipeline stage.
13. External outer PSF paths supplied through the main script are reused for each selected join operation.
14. Custom GNUastro arguments are passed directly to the corresponding GNUastro commands.
15. A circular PSF radius must be odd.
...
Check that `--dir` points directly to the directory containing `.fits` files.

### A filter is ignored

Check both the filename and `--filters`. For example, `NGC1037_g.fits` requires `--filters g` or a list containing `g`.

### Gaia query fails

Check internet connectivity, `astquery`, and the celestial WCS.

```bash
astquery --version
```

### Galaxy coordinate lookup fails

The object name extracted from the filename must work with `SkyCoord.from_name`.

### PSF builder reports a missing Gaia match

The builder expects:

```text
Process_data/Make_catalogs/Data_catalog_gaia_match/match_gaia_<GALAXY>_<FILTER>.fits
```

Run `--make-catalogs` first.

### PSF builder reports a missing segmentation file

The builder expects:

```text
Process_data/Make_catalogs/Mask_segment/seg-<INPUT_FILENAME>
```

These files are generated by the catalogue stage.
...
```bash
which ds9
astscript-fits-view --help
```

### Matplotlib/LaTeX error in the joiner

Install a suitable LaTeX distribution or disable `plt.rcParams["text.usetex"]` in `psf_joint.py`.

### `circular_radius must be ODD`

Use an odd integer such as `1001`, `2001`, or `6801`.

### No final joined profile is produced

In the interactive joiner, define overlap intervals, press **Join**, verify that joining diagnostics appear, and then close the window.

### GNUastro command not found
...
which astsegment
which astmkcatalog
which astscript-psf-stamp
```

The tested GNUastro version is `0.23`.

---

## 🏷️ Versioning

LISAN uses semantic-style version tags. The current release is:
...
For reproducible scientific work, report the exact LISAN release used and prefer tagged GitHub releases over the continuously changing development branch.
